...
def run_bot(bot, connection_args, verbose=False):
    if verbose:
        print('Connecting to %s:%d' % (connection_args.host, connection_args.port))

    try:
        sock = socket.create_connection((connection_args.host, connection_args.port))
    except socket.error as e:
        print('Error connecting to {}:{}. Aborting.'.format(connection_args.host, connection_args.port))
        exit()

    socketfile = sock.makefile()
    runner = Runner(bot, socketfile, verbose)
    runner.run()
    socketfile.close()
